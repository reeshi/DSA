...
public class LongestConsecutiveSequence {
	...
	public static ArrayList<Integer> longestConsecutiveIncreasingSequence(int[] arr) {
        ...
        for(int i=0; i<arr.length; i++){
            int len = 0;
            int temp_start = arr[i];
            
            while(map.containsKey(temp_start) && map.get(temp_start) == true){
                map.put(temp_start, false);
                len++;
                temp_start++;
            }
            
            temp_start = arr[i];
            while(map.containsKey(temp_start-1) && map.get(temp_start-1) == true){
                map.put(temp_start-1, false);
                len++;
                temp_start--;
            }
